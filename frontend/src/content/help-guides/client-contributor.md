# Client Contributor Guide

As a Client Contributor, your account is set up to eventually let you add and edit information on the projects you have access to, not just view it. Right now, the screens you see are the same as a Client Viewer's. There's no Add Task or edit button available yet on client accounts of any kind. This guide covers what you can do today and what to expect once editing is available.

> **Note:** This is a known gap between what your account is permitted to do and what the interface currently exposes. Nothing is broken on your end; there's no button yet for the extra access your role grants. This guide will be updated once that changes.

## Getting access

There's no self-service sign-up for a Client account. A Project Manager or Admin on the project invites you by email and assigns you the Contributor role, and you're given login credentials directly. If you're expecting access and haven't received credentials, check with your contact on the project team.

## Signing in

![The iTrack sign-in card, with Email and Password fields and a Sign in button](images/login.png)

1. Go to the iTrack login page.
2. Enter your **Email** and **Password**.
3. Select **Sign in**.

If you get "Invalid credentials," double check what was sent to you, and confirm you're using the right link. This isn't a self-registration page.

## Your dashboard

After signing in, you land on the **Dashboard**. It shows overall progress on the projects you have access to, a status breakdown (Completed, In Progress, Not Started, Delayed), a heatmap of activity by module, and a feed of recent updates.

![The Dashboard, showing the Overall Progress ring, Task Status cards, and Project Structure cards](images/client-dashboard.png)

## Viewing project progress in Work Program

**Work Program** (sidebar) shows the project structure (**Module → Activity → Sub-Activity → Task**) for whatever's been shared with you.

![The Work Program list view for a Client account, showing tasks with Shared with Client badges, status, and progress, but no Responsible or Contributor columns](images/client-work-program.png)

1. Select a project, then switch between **List** and **Gantt** view.
2. Use **Expand All** / **Collapse All** to open or close everything at once, and the **Status** filter to narrow the list (Backlog, Not Started, In Progress, For Review, Completed, Blocked, Delayed).
3. In Gantt view, you can toggle **Baseline** (planned vs. actual dates) and **Critical Path** (which tasks are driving the end date), and use **Export PDF** to save a copy.

A few things are specific to your view:

- You only see tasks the project team has marked **Share with Client**. This is normal; if you're expecting to see something that isn't there, ask your project contact whether it's been shared yet.
- You won't see the **Responsible** or **Contributor** columns. Internal staffing assignments aren't shown to client accounts.
- If you need to add information, flag a task, or request a change, send it to your project contact directly for now rather than looking for an edit option in the interface.

## Checking project timing

**Schedule View** (sidebar) gives you a simpler, calendar-style view of the same activities and milestones, by month or week.

![The Schedule View calendar, showing tasks plotted on their planned dates for the month](images/client-schedule-view.png)

## Viewing project health

**Reports & Health** (sidebar) shows the current health status of your project along with any notes the project team has added. This is view-only for your account.

## Pages you won't see

**Kanban Board** and **Support Ops** don't appear in your sidebar. They're internal-only tools for the vendor team. If you land on one of those URLs directly, you'll see a message explaining that client accounts don't have access.

![The Access Denied page shown when a Client account visits an internal-only page like the Kanban Board](images/access-denied.png)
