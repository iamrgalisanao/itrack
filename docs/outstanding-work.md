...
## Found while implementing 024 Story 1

| Item | Status | Notes |
|---|---|---|
| **`isClient` fails open and survives at 12 sites outside the Gantt** — List view's contributor column (`WorkProgram.jsx:1936`, `:2187`), taskboard routing, client-visibility filters | **OPEN** | C2 replaced the five **Gantt** sites with `canSeeContributor`, a positive allowlist. The rest still use `userRole === 'Client'`, which returns `false` for `null` — and `useEffectiveUser()` returns `null` until auth resolves, so a viewer whose role is not yet known is treated as non-Client. T065 names the three Gantt sites (it undercounts: the two column-span ternaries are layout consequences of the same decision and must move with them). The List-view contributor column is the same defect class and no 024 requirement covers it. A two-sided ratchet in `WorkProgram.contributorGate.test.js` holds the count at 12 so it cannot grow while it waits |
| **The seeded Client has no project assignment**, so no Client-facing view can be observed against seeded data without granting one | **OPEN — dev-data gap, not a defect** | C2's live check had to create a temporary `ProjectAssignment` (project 1, user 6, `assigned_by_user_id` required) and delete it afterwards; the table is back to 0 rows. Every future Client-facing verification pass will pay the same cost. Worth seeding a Client assignment in `DatabaseSeeder` so the Client persona is actually exercisable |

## Found while implementing 024 Story 3

| Item | Status | Notes |
|---|---|---|
| **`getHealthStyle` in `Reports.jsx:216` is a second status vocabulary with a fail-open default** — its `default:` branch returns `{ label: 'On Track', color: 'bg-emerald-500/10 …' }`, so an unknown, new or null project health renders as **healthy and green, with a label that says so** | **OPEN** | FR-022 ("no status may reach its visual treatment through a default branch") covers this in principle. Story 3's gate does **not**: it is anchored to `STATUS_ORDER`, and project health is a separate backend enum. Recorded rather than fixed in passing — the label lies, not only the colour, so it deserves its own look at what the correct unknown-health treatment is, not a drive-by edit inside a chart PR. It also carries 15 of the 27 raw palette literals remaining in the file |
| **27 raw palette literals remain in `Reports.jsx`** (health badges, the milestone dot, the completed/pending milestone pills) | **OPEN** | Deliberately not covered by Story 3's ratchet row, which is scoped to `StatusBreakdownPanel` at 0. A file-wide ban would have been red on day one, and the recorded consequence of that is an implementer weakening the regex — which is how coverage is lost. Same reasoning that scoped the existing ratchet to named maps rather than to `WorkProgram.jsx` as a whole |
| **`@media print` flattens `.flex` to `display: block !important`, which un-blockifies inline children** | **CLOSED for the chart, OPEN app-wide** | Story 3 hit this: the status chart printed as seven empty rails because its mark `<span>`s reverted to `display: inline`, where a percentage width does nothing. Fixed there by declaring `block` on the marks. **The reset is app-wide and every other `.flex` row on a printable page has the same exposure** — the Risk Flags rows were moved to grid in the same change, but nothing systematically checks the rest. `print_check.py` (uncommitted, `scratchpad/`) is the shape a gate for this would take; it needs the app running and a login, which `verify-cascade.py` structurally does not do |
...
## Found while building 024 Story 1's announcement (C4)

| Item | Status | Notes |
|---|---|---|
| **FR-007 is enforced server-side; the frontend gate is defence in depth** | **CLOSED as a clarification** | Measured against the API, not inferred: a Client's `GET /projects/{id}/modules` omits `responsible` **entirely** — the key is absent, not null — while an Admin receives its values. So `canSeeContributor` and `showContributor` are a second layer, not the control. Worth recording because it changes how much weight the frontend gate carries, and because **it makes the "a Client description must not leak the contributor" assertion vacuous by construction**: there is no contributor in the Client's payload to leak. Tampering `includeContributor` to a hard `true` left the check green — the correct result, and a useless one. `gantt_announcement.py` now asserts the **API response** (non-vacuous, and where the control actually lives) and keeps the rendered-text assertion with its vacuity stated at the assertion, because it stops being vacuous the day the resource starts sending the field |
| **A check reported three failures this increment that were its own construction, not the app's** | **CLOSED — all three fixed** | (1) bars selected by their focus-outline class, so deleting the outline reported *"no bars found"*; (2) exposed-node count compared against **bar** count when rows without dates render a row and no bar, inventing a duplicate announcement out of 8-vs-4; (3) card opacity read in the same frame as the focus, mid-`duration-200`, reporting the reveal as broken. Each was caught by asking why a failure looked odd rather than by changing the app to satisfy it. Recorded as a pattern: **on this surface, a surprising red is more often the instrument than the subject** |

## Found while building 024 Story 1's keyboard support

| Item | Status | Notes |
|---|---|---|
| **Headless Chromium does not report author outline width, colour or offset** | **CLOSED as an instrument defect - recorded so nobody re-discovers it** | On a keyboard-focused element, `getComputedStyle` returns `outline-width`, `outline-color` and `outline-offset` pinned at their initial values (`medium` / `currentColor` / `0`) no matter what is set - *including inline*, which no author rule can override. Setting `outline: 4px dashed red` inline computes as `dashed 3px currentColor offset 0`: **only `outline-style` responds.** Headed, the same build reports `solid 2px rgb(180,83,255) offset 1.33px`. This cost an afternoon: the bar's focus utilities were declared broken, replaced by an unlayered `index.css` rule, and that "did not work either" - because the instrument could not see either of them. Both were always correct. `scripts/uichecks/gantt_keyboard.py` now asserts `outline-style` only and carries the reason at the assertion. **Any future focus-indicator work must be verified headed or by eye.** |
| **A stale dev server can silently serve a different build on the port the harness targets** | **OPEN - environment hazard** | Vite binds 5174, 5175, … when 5173 is taken and says so only in its startup banner; the harness still targets 5173 and still passes, because a stale server started from the same checkout serves the same files over HMR. It stops being harmless when the stale server outlives its watcher or runs from a different checkout - then the checks measure code nobody is looking at, and pass. Recorded in `scripts/uichecks/README.md`; the durable fix would be for the harness to assert the served build matches the working tree |
