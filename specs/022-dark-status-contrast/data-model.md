...
Plus **composited tints**: 26 call sites across 15 files render `text-{state}` directly on a
same-colour tint (`bg-{state}/10`, `/15`), which is a surface in none of the three rows above. These are part of
...
| State | `--{state}` | as text | on tint | `--{state}-foreground` | as fill |
|---|---|---|---|---|---|
| destructive | `#dc2626` → **`#f87171`** | 3.36 → **5.86** | 3.03 → **4.67** | `#ffffff` → **`#16171d`** | 4.83 → **6.46** |
| success | `#15803d` → **`#4ade80`** | 3.23 → **9.30** | 2.83 → **6.78** | `#ffffff` → **`#16171d`** | 5.02 → **10.26** |
| warning | `#b45309` → **`#fbbf24`** | 3.23 → **9.71** | 2.83 → **6.94** | `#ffffff` → **`#16171d`** | 5.02 → **10.71** |
| info | `#2563eb` → **`#60a5fa`** | 3.14 → **6.38** | 2.75 → **4.92** | `#ffffff` → **`#16171d`** | 5.17 → **7.03** |
...
- **4 call sites** (all `frontend/src/components/MyWorkPanel.jsx`) currently override the token in
  ...
  similar `dark:text-*-400` occurrences are **not** in scope.
- **1 call site** (`frontend/src/pages/Schedule.jsx:655`) hard-codes `text-white` on
  `bg-destructive`. Inert today — the branch renders no text — but it would measure 2.77:1 against
  the new dark fill, so the dead class is deleted rather than left as a trap.
