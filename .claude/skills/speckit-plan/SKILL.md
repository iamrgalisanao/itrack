...
- **Both** `/speckit-tasks` and `/speckit-implement` MUST refuse to proceed against a plan.md missing that section or recording anything other than `PASSED` — `/speckit-tasks` at its step 3a, `/speckit-implement` at its step 3a. Gating only at implement is not sufficient: by then `tasks.md` already exists carrying the unverified plan's errors, and correcting the plan afterwards leaves a stale task list beside it with nothing forcing regeneration.
