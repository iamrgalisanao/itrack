...
7. **Mandatory Software Architect verification of the generated artifacts**: After the post-design Constitution Check, and before the Completion Report, the orchestrating session MUST dispatch a **Software Architect** subagent (Agent tool, `subagent_type: "Software Architect"`) to independently verify the plan artifacts. This is automatic — the user does not need to ask for it.
   ...
   - The orchestrating session MUST resolve every Misaligned/Vague finding by revising the artifacts and re-dispatching verification on the changed surface, OR document the finding and the reason it is accepted in plan.md (same exception mechanism as a Constitution Check violation). Completion MUST NOT be reported while unresolved, undocumented findings remain.
   - If subagent dispatch is unavailable in the current environment, run the identical checklist as a dedicated inline verification pass and note in plan.md that verification ran inline — the gate itself is never skipped.
...
- Every plan run ends with the Software Architect verification gate (step 7) — the plan artifacts are not final until verified aligned with spec.md and the constitution, and concrete (no vague placeholders or unresolved clarifications), with any accepted findings documented in plan.md

## Done When

- [ ] Plan workflow executed and design artifacts generated
- [ ] Software Architect verification (step 7) dispatched, and every Misaligned/Vague finding resolved or documented as accepted in plan.md
